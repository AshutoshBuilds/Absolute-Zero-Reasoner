...
def _quality_hint(text: str) -> tuple[int, int, dict[str, bool]]:
    """
    Lightweight heuristic used only for smoke diagnostics (not an oracle).

    The goal is to catch obvious degenerate outputs without replacing manual
    review for actual mathematical correctness.
    """
    checks = {
        "halting mention": "halt" in text.lower() and "tm" in text.lower(),
        "A_TM mention": "a_tm" in text.lower(),
        "acceptance mention": "accept" in text.lower() and "tm" in text.lower(),
        "rice mention": "rice" in text.lower(),
        "reduction mention": "reduction" in text.lower(),
        "security mention": "security" in text.lower() or "safety" in text.lower(),
        "compiler mention": "compiler" in text.lower() or "static" in text.lower(),
    }
