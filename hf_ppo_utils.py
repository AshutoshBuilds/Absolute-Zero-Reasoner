...
def get_model_outputs_for_ppo(trainer_instance,
    ...
    if not adapter.use_separate_value_model:
        logger.error("Cannot get PPO outputs: Adapter not using separate actor/critic models.")
    ...
    current_full_sequence_mask = full_sequence_mask
    if torch.is_autocast_enabled() and device.type == 'cuda':
        if current_full_sequence_mask.dtype != torch.float32:
            logger.debug(f"_get_model_outputs_for_ppo: Casting full_sequence_mask from {current_full_sequence_mask.dtype} to torch.float32 for adapter call under autocast.")
            current_full_sequence_mask = current_full_sequence_mask.to(torch.float32)

    raw_full_logits, log_probs_per_token, values = adapter.get_action_and_value(
        input_ids=full_sequence_ids,
        attention_mask=current_full_sequence_mask,
        actions=action_ids
    )
...
def perform_ppo_update(trainer_instance) -> dict:
    ...
    for ppo_epoch_num in range(config["ppo_epochs"]):
        ...
        for start_idx in range(0, num_samples, config["batch_size"]):
            ...
            with autocast(device_type=device.type, enabled=use_mixed_precision):
                ...
                use_mixed_precision = (device.type == 'cuda' and scaler is not None)
                model_dtype = torch.float16 if use_mixed_precision else torch.float32
                ...
                actor_original_mode_is_train = adapter.actor_model.training
                critic_original_mode_is_train = adapter.critic_model.training
                if actor_original_mode_is_train: adapter.actor_model.eval()
                if critic_original_mode_is_train: adapter.critic_model.eval()
                ...
                if actor_original_mode_is_train: adapter.actor_model.train()
                if critic_original_mode_is_train: adapter.critic_model.train()
                ...
                if current_action_logits_for_actions.nelement() > 0:
                    ...
                    try:
                        dist = D.Categorical(logits=current_action_logits_for_actions)
                ...
                surr2 = torch.clamp(ratio, 1.0 - config["ppo_clip_epsilon"], 1.0 + config["ppo_clip_epsilon"]) * mb_advantages
                policy_loss = -torch.min(surr1, surr2).mean()

                value_pred_clipped = mb_old_values + torch.clamp(current_values - mb_old_values, 
                                                               -config["value_clip_epsilon"], 
                                                               config["value_clip_epsilon"])
                value_loss_unclipped = F.mse_loss(current_values, mb_returns)
                ...
                if torch.isnan(loss).any() or torch.isinf(loss).any():
                    logger.error("CRITICAL: NaN or Inf detected in total PPO loss (inside autocast). Skipping backward pass for this minibatch.")
                    continue
            ...
            if adapter.use_separate_value_model:
                ...
                if hasattr(trainer_instance, 'optimizer_params') and trainer_instance.optimizer_params: # Assuming optimizer_params is defined in HuggingFaceRLTrainer
                     for p in trainer_instance.optimizer_params:
                        if p.grad is not None: torch.nn.utils.clip_grad_value_(p, gradient_clip_value)
            ...
            if adapter.use_separate_value_model:
                ...
                if hasattr(trainer_instance, 'optimizer_params') and trainer_instance.optimizer_params:
                    torch.nn.utils.clip_grad_norm_(trainer_instance.optimizer_params, gradient_clip_norm)
    ...
    num_updates = config["ppo_epochs"] * (num_samples // config["batch_size"] + int(num_samples % config["batch_size"] != 0) )
    ...
    return {"avg_policy_loss": avg_policy_loss, "avg_value_loss": avg_value_loss, "avg_entropy": avg_entropy} 